...
def compute_pad_metrics(probabilities: np.ndarray, labels: np.ndarray) -> dict:
    ...
    ViTUnified PAD head outputs num_classes logits; the caller should pass
    softmax / sigmoid probabilities indicating the spoof class probability.
...
def compute_identification_metrics(
    ...
    top_scores = sim_mat[rows, top_indices_unsorted]

    # Sort only the selected top max_k candidates.
    order = np.argsort(-top_scores, axis=1)
    rows = np.arange(top_indices_unsorted.shape[0])[:, None]
    top_indices = top_indices_unsorted[rows, order]
