...
def collect_authentication_scores_trt(engine, authentication_loader, unique_loader, embed_dim):
    ...
    for idxs, imgs in tqdm(unique_loader, desc="TensorRT Extracting Embeddings", unit="batch"):
        ...
        # embedding is the CLS-token recognition output from ViTUnified
        embedding, _ = engine.infer(imgs_np)

        embs = embedding.reshape(imgs_np.shape[0], -1)
...
def evaluate_recognition_trt(engine, cfg, recog_split_path):
    ...
    # ViTUnified backbone embed_dim is inferred from the engine output shape
    # at runtime; use a placeholder here so the pre-allocation is sized correctly.
    # Run a single dummy inference to get the actual embed_dim.
    import numpy as _np
    _dummy = _np.zeros((1, 3, 224, 224), dtype=_np.float32)
    _emb, _ = engine.infer(_dummy)
    embed_dim = _emb.shape[-1]
...
def run_recognition_evaluation(
    ...
    output_dir="results/vu_trt_recog",
):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default="results/vu_trt_recog")
